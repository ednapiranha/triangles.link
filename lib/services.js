...
exports.room = function (request, reply) {
  setContext(request);
  ctx.firstVisit = false;

  if (ctx.session && ctx.uid === request.params.uid) {
    ctx.owner = true;
    ctx.firstVisit = request.auth.credentials.firstVisit;
  } else {
    ctx.owner = false;
  }
  ...
  rooms.get(request.params.uid, (err, roomData) => {
    ...
    ctx.level = roomData.level || false;

    reply.view('dashboard', ctx);

    if (ctx.owner) {
      authenticate.removeFirstVisit(request.auth.credentials.email, () => {
        request.auth.credentials.firstVisit = false;
      });
    }
  });
};
